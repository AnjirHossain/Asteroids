...
public class Game extends Applet implements Runnable, KeyListener {
	...
	public static void main(String[] args) {
        ...
        final double rateOfFire = 3; //limits rate of fire
        ...
        private final int frameW = 900, frameH = 700; 
        
        public void init() {
                resize(frameW,frameH); //set size of the applet
                dim = getSize(); //get dimension of the applet
                width = dim.width;
                height = dim.height;
                framePeriod = 28; //set refresh rate
                
                addKeyListener(this); //to get commands from keyboard
                setFocusable(true); 
                
                ship = new SpaceShip(width/2, height/2, 0, .15, .9, .15, .98); //add ship to game
                shipCollision = false;
                shipExplode = false;
        ...
        g2d.setColor(new Color(54, 215, 183));
        g2d.fillRect(0, 0, width, height); //adds back ground 
